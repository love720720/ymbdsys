...
<script type="text/javascript" src="${pageContext.request.contextPath }/js/page/classify-edit.js"></script>
<style type="text/css">
.width_300{
	width: 300px;
}
...
<body>
<div class="alert alert-info">当前位置<b class="tip"></b>分类管理<b class="tip"></b><c:if test="${empty classify }">分类添加</c:if><c:if test="${! empty classify && classify.id > 0}">分类修改</c:if></div>
<center>
 <form method="post" action="editClassify.htm" id="form_edit">
  <input type="hidden" name="id" value="${classify.id }"/>
  <table class="width_300 table table-striped table-bordered table-condensed">
   <tr>
    <th width="22%">名称</th>
	<td><input type="text" id="name" name="name" value="${classify.name }"/></td>
   </tr>
	<tr>
	<th>排序</th>
	<td><input type="text" id="sort" name="sort" onkeyup="this.value=this.value.replace(/[^\d]/g,'')" value="${classify.sort }"/></td>
   </tr>
   <tr>
