...
<script type="text/javascript" src="${pageContext.request.contextPath }/js/page/classify-list.js"></script>
<style type="text/css">
td[name='textCenter']{
	text-align: center;
}
...
<body>
<div class="alert alert-info">当前位置<b class="tip"></b>分类管理<b class="tip"></b>分类列表</div>
 <table class="table table-striped table-bordered table-condensed">
  ...
  <tr>
   <td>${classify.name }</td>
   <td>第${classify.sort }位</td>
   <td>
   <c:forEach items="${buttonList }" var="button">
	<c:if test="${button.code == 1001 }">
     <a href="javascript:edit('${classify.id }');" title="点击修改" class="btn btn-mini btn-danger">修改</a>&nbsp;&nbsp;
	</c:if>
	<c:if test="${button.code == 1002 }">
     <a href="javascript:del('${classify.id }');" title="点击删除" class="btn btn-mini btn-danger">删除</a>
	</c:if>
   </c:forEach>
   </td>
  </tr>
  ...
  <c:if test="${empty list || list.size() <= 0 }">
  <tr><td name="textCenter" colspan="3">暂无信息</td></tr>
  </c:if>
 </table>
</body>
</html>